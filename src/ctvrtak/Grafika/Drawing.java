...
//        drawShapes(g2);
        ...
        drawButterfly(g2, 100, 100, 40);
    }

    void drawButterfly(Graphics2D g, int x, int y, int length) {
        g.setStroke(new BasicStroke(3));

        Path2D butterfly = new Path2D.Double();

        butterfly.moveTo(x, y);
        butterfly.lineTo(x+2*length, y+length);
        butterfly.lineTo(x+2*length, y);
        butterfly.lineTo(x, y+length);
        butterfly.closePath();
